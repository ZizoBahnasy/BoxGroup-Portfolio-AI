...
class PortfolioFunctions(llm.FunctionContext):
    def __init__(self):
        super().__init__()
        kb_path = Path(__file__).parent.parent / "data" / "portfolio_4_valuations.json"
        ...
        self.bg_style = (
            "BoxGroup Investment Style:\n"
            "1. Early-Stage Focus: Invest in companies at seed and pre-seed stages\n"
            "2. Category-Defining: Back businesses that can define or redefine their categories\n"
            "3. Technology-Driven: Support companies using technology as a catalyst for change\n"
            "4. Strong Teams: Partner with exceptional founders who can execute on ambitious visions"
        )
        self.personal_style = (
    ...
    async def get_investment_style(
        ...
        if style_type.lower() == "boxgroup":
            return self.bg_style
        elif style_type.lower() == "personal":
            return self.personal_style
        else:
            return f"BoxGroup Investment Style:\n{self.bg_style}\n\nPersonal Investment Style:\n{self.personal_style}"
    ...
    async def check_thesis_alignment(
        ...
        for company in self.portfolio_data:
            if company_name in company["name"].lower():
                if thesis_type.lower() == "boxgroup":
                    return (f"BoxGroup Thesis Alignment for {company['name']}:\n"
                            f"Score: {company['boxgroup_thesis_alignment']['score']}/10\n"
                            f"Analysis: {company['boxgroup_thesis_alignment']['description']}")
                    ...
                    return (f"Thesis Alignment for {company['name']}:\n\n"
                            f"BoxGroup Thesis:\nScore: {company['boxgroup_thesis_alignment']['score']}/10\n"
                            f"{company['boxgroup_thesis_alignment']['description']}\n\n"
    ...
    def _parse_extended_nl_criteria(self, query: str) -> dict:
        ...
        # --- Thesis Alignment (BoxGroup and Personal) ---
        m = re.search(r'(?:boxgroup thesis alignment|boxgroup)\s+(?:above|over|greater than)\s*([\d\.]+)', q_lower)
        if m:
            criteria["min_boxgroup_thesis"] = float(m.group(1))
        ...
        if m and ("min_boxgroup_thesis" not in criteria and "min_personal_thesis" not in criteria):
            val = float(m.group(1))
            criteria["min_boxgroup_thesis"] = val
            criteria["min_personal_thesis"] = val
        m = re.search(r'boxgroup thesis description\s+(?:contains|includes)\s+"([^"]+)"', q_lower)
        if m:
            criteria["boxgroup_thesis_description_contains"] = m.group(1).strip()
    ...
    def _apply_extended_criteria(self, criteria: dict) -> list:
        ...
        for company in self.portfolio_data:
            ...
            # --- BoxGroup Thesis Alignment ---
            if "min_boxgroup_thesis" in criteria:
                score = company.get("boxgroup_thesis_alignment", {}).get("score", 0)
                if score < criteria["min_boxgroup_thesis"]:
                    keep = False
            if "boxgroup_thesis_description_contains" in criteria:
                text = company.get("boxgroup_thesis_alignment", {}).get("description", "").lower()
                if criteria["boxgroup_thesis_description_contains"].lower() not in text:
                    keep = False
    ...
    @llm.ai_callable(
        description=(
            ...
            "'personal_thesis' (zizo), 'boxgroup_thesis', 'defensibility', 'market_saturation', "
    ...
    async def rank_companies(
        ...
        field_mapping = {
            ...
            "boxgroup_thesis": lambda comp: comp.get("boxgroup_thesis_alignment", {}).get("score", 0),
    ...
    @llm.ai_callable(
        description="Calculate the average BoxGroup thesis alignment score, optionally filtered by a natural language query."
    )
    async def calculate_average_boxgroup_thesis(self, filter_query: str = "") -> str:
        companies = self.portfolio_data
        if filter_query:
            criteria = self._parse_extended_nl_criteria(filter_query)
            companies = self._apply_extended_criteria(criteria)
        scores = [comp.get("boxgroup_thesis_alignment", {}).get("score") for comp in companies
                  if comp.get("boxgroup_thesis_alignment", {}).get("score") is not None]
        if not scores:
            return "No BoxGroup thesis data available."
        avg = sum(scores) / len(scores)
        return f"The average BoxGroup thesis alignment score is {avg:.2f}/10."
    ...
    @llm.ai_callable(
        description=(
            ...
            "If the query mentions specific fields (e.g. 'defensibility', 'investment thesis', 'boxgroup', 'personal', 'excitement', "
    ...
    async def get_company_specific_info(self, query: str) -> str:
        ...
        elif "boxgroup" in q_lower:
            box_info = target_company.get("boxgroup_thesis_alignment", {})
            if not box_info:
                return f"I'm sorry, I don't have BoxGroup thesis alignment info for {target_company['name']}."
            output = f"BoxGroup Thesis Alignment for {target_company['name']}:\n"
        ...
        summary += f"BoxGroup Thesis Alignment: Score {target_company.get('boxgroup_thesis_alignment', {}).get('score', 'N/A')} - {target_company.get('boxgroup_thesis_alignment', {}).get('description', 'N/A')}\n"
    ...
    async def get_company_info_nl(
        ...
        if "note" in query_lower:
            ...
            return (f"The investment thesis for {candidate['name']} is as follows: "
                    f"{candidate.get('boxgroup_thesis_alignment', {}).get('description', 'No thesis available')}")
